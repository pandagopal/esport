...
contract MigrationAgentInterface {
  ...
  function setSourceToken(address _dttSourceToken) public;
  function updateSupply() public;
  function dttSourceToken() public returns (address);
}

/**
   @title DTT, the DreamTeam token

   Implementation of DTT, an ERC20 token for the DreamTeam ecosystem. The smallest unit of a dtt is
   the atto. The token call be migrated to a new token by calling the `migrate()` function.
 */
contract DreamTeamToken is BurnableToken, PausableToken, VestedToken, MintableToken {
     ...
     * Constructor.
     */
    function DreamTeamToken(address _migrationMaster) public {
    ...
    */
    function mintVestedTokens (address _to, uint256 _value,
      uint64 _start,
      uint64 _cliff,
      uint64 _vesting,
      bool _revokable,
    ...
    */
    ...
    function setMigrationAgent(address _agent) public onlyMigrationMaster {
      require(MigrationAgentInterface(_agent).dttSourceToken() == address(this));
    ...
    function mint(address _to, uint256 _amount) onlyOwner canMint public returns (bool) {
      ...
      bool minted = super.mint(_to, _amount);

      if (minted && migrationAgent != address(0)) {
     ...
     */
    function resetNewTokens()public {
     ...
     */
    function resetBurntTokens()public {
      require(msg.sender == migrationAgent);
      burntTokens = 0;
    }
