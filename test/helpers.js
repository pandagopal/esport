...
var DTCrowdsale = artifacts.require('./DTCrowdsale.sol');
var abiDecoder = require('abi-decoder');
abiDecoder.addABI(DTToken._json.abi);
abiDecoder.addABI(DTCrowdsale._json.abi);

var latestTime = require('./helpers/latestTime');
var {increaseTimeTestRPC, increaseTimeTestRPCTo} = require('./helpers/increaseTime');

const DEBUG_MODE = (process.env.DTT_DEBUG == 'true' || process.env.npm_config_dtt_debug == 'true') || false;
...
module.exports = {
  ...
  simulateCrowdsale: async function(rate, goal, cap, minInvest, maxCumulativeInvest, maxGasPrice, minBuyingRequestInterval, accounts, balances, finish) {
    ...
    var crowdsale = await DTCrowdsale.new(
      startTime, endTime,
      rate,
      goal, cap,
