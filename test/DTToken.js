...
var DreamTeamToken = artifacts.require('DreamTeamToken.sol');
var DreamTeamMigrationToken = artifacts.require('DreamTeamMigrationToken.sol');
var MigrationAgent = artifacts.require('MigrationAgent.sol');


contract('dreamteamToken', function(accounts) {

  let token, targetToken, migrationAgent,
    migrationMaster = accounts[5];

  beforeEach(async function() {
    token = await DreamTeamToken.new(migrationMaster);
  ...
  it('has name, symbol and decimals', async function() {
    assert.equal('DTT', await token.symbol());
    assert.equal('dreamteam token', await token.name());
    assert.equal(6, await token.decimals());
  });

  it('can NOT create a qiibee token with no migration master', async function() {
    try {
      await DreamTeamToken.new();
  ...
  describe('burning tokens', async () => {
    ...
    beforeEach(async function() {
      ...
      targetToken = await DreamTeamMigrationToken.new(migrationAgent.address);
  ...
  describe('migrating tokens', async () => {
    ...
    it('can NOT finalize migration until all tokens have been migrated', async () => {
      ...
      try {
        ...
        assert.notEqual(help.zeroAddress, await migrationAgent.dttSourceToken());
        assert.notEqual(help.zeroAddress, await migrationAgent.dttTargetToken());
        assert.notEqual(help.zeroAddress, await migrationAgent.tokenSupply());
        if (!help.isInvalidOpcodeEx(error)) throw error;
      }
      await token.migrate(web3.toWei(1), {from: accounts[1]});
      await migrationAgent.finalizeMigration({from: accounts[0]});
      assert.equal(help.zeroAddress, await migrationAgent.dttSourceToken());
      assert.equal(help.zeroAddress, await migrationAgent.dttTargetToken());
